...
namespace Mirror
{
    ...
    public class NetworkRoomManager : NetworkManager
    {
        ...
        void SceneLoadedForPlayer(NetworkConnectionToClient conn, GameObject roomPlayer)
        {
            ...
            RoomManager.s_idIterator++;
            if (roomPlayer.GetComponent<LobbyUI>() == null) Debug.LogError("LobbyUI is null");
            roomPlayer.GetComponent<LobbyUI>().m_currentPlayerId = RoomManager.s_idIterator;
            Debug.Log("Current Player ID: " + roomPlayer.GetComponent<LobbyUI>().m_currentPlayerId);
